...
function ApplicationCard({ item }: { item: AdminSubmitApplication }) {
    ...
    const firstTarget = item.consultationTargets?.[0];
    const targetSummary = item.consultationTargets?.length
        ? `${firstTarget?.name || '상담대상'}${item.consultationTargets.length > 1 ? ` 외 ${item.consultationTargets.length - 1}명` : ''}`
        : '상담대상 없음';

    return (
        <article className="rounded-lg border border-stone-200 bg-white shadow-sm transition-shadow duration-200 hover:shadow-md">
            <div className="grid gap-4 p-4 lg:grid-cols-[minmax(180px,1.2fr)_minmax(220px,1.4fr)_minmax(220px,1.8fr)_auto] lg:items-center">
                <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                        <span className="text-sm font-semibold text-stone-400">#{item.id}</span>
                        <span className="inline-flex h-7 items-center rounded-full bg-amber-50 px-2.5 text-xs font-semibold text-amber-700">
                            {serviceTypeLabels[item.serviceType] || item.serviceType}
                        </span>
                    </div>
                    <h3 className="mt-1 break-words text-xl font-bold text-stone-800">{item.name}</h3>
                    <p className="mt-1 text-sm font-medium text-stone-500">{item.contact}</p>
                </div>

                <div className="grid gap-1.5 text-sm text-stone-600 sm:grid-cols-3 lg:grid-cols-1">
                    <CompactInfo label="접수일" value={formatDate(item.createdAt)} />
                    <CompactInfo label="상담대상" value={targetSummary} />
                    <CompactInfo label="상담방법" value={formatConsultationMethod(item.serviceDetails?.consultationMethod)} />
                </div>
            ...
            {isExpanded && (
                <div className="border-t border-stone-100 px-4 pb-5 pt-4">
                    <div className="grid gap-4 rounded-lg border border-stone-100 bg-stone-50/70 p-4 text-sm text-stone-600 sm:grid-cols-2 lg:grid-cols-4">
                            ...
                            {item.consultationTargets?.length ? (
                                item.consultationTargets.map((target, index) => (
                                    <div key={`${target.name || 'target'}-${index}`} className="rounded-lg border border-stone-100 bg-white p-4">
                                        ...
                                        <dl className="mt-3 grid gap-2.5 text-sm text-stone-600">
                                ...
                                <p className="rounded-lg border border-stone-100 bg-white p-4 text-sm text-stone-500">
                                    상담대상 정보가 없습니다.
                                </p>
                            )}
                        </div>
                    ...
                    <section className="mt-5">
                        ...
                        <dl className="mt-3 grid gap-2.5 rounded-lg border border-stone-100 bg-white p-4 text-sm text-stone-600 md:grid-cols-2">
                    ...
                    <section className="mt-5">
                        ...
                        <p className="mt-3 whitespace-pre-wrap break-words rounded-lg bg-stone-50 px-4 py-3 text-sm leading-7 text-stone-600 [overflow-wrap:anywhere]">
...
function CompactInfo({ label, value }: { label: string; value?: string | null }) {
    return (
        <div className="min-w-0">
            <span className="mr-2 font-semibold text-stone-400">{label}</span>
            <span className="break-words font-medium text-stone-700 [overflow-wrap:anywhere]">{value || '-'}</span>
        </div>
    );
}
